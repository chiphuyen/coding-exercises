...
function that takes as input an array and outputs the sum of all of its subarrays. For example,
given [1, 3, 7], you'd output 36, because
[ ] + [1] + [3] + [7] + [1, 3] + [3, 7] + [1, 3, 7] 
= 0 + 1 + 3 + 7 + 4 + 10 + 11 = 36

My solution:
	subarray_sum_slow: the easy but slow way to do it. Just enumberate all the subarrays 
					   and sum them up
	subarray_sum: the fast way to do it. We can calculate the number of times each element occurs
				  and sum them up.
'''
...
assert subarray_sum_slow([1, 3, 7, 9, 11, 13]) == subarray_sum([1, 3, 7, 9, 11, 13])