...
Write a function that takes as input a string and returns whether the parenthesis are "balanced". 
If the candidate solves this question quickly, add the following difficulty: 
the string may also contain "[]" or "{}". Return whether all three types of brackets are balanced.
...
assert is_balanced('(()', delims=['('], closes=[')']) == False
assert is_balanced('(())', delims=['('], closes=[')']) == True
assert is_balanced('()(()())', delims=['('], closes=[')']) == True
assert is_balanced('()(()())()', delims=['('], closes=[')']) == True
assert is_balanced('(()())', delims=['('], closes=[')']) == True
assert is_balanced('(()))', delims=['('], closes=[')']) == False
